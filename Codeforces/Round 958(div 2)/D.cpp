...
void calc(ll node, ll par, vvl &dp, vl &val,vl adj[]){
    for(int i=0;i<22;i++){
        dp[node][i] = (i+1)*val[node];
    }
    for(auto child: adj[node]){
        if(child  == par) continue;
        calc(child,node,dp,val,adj);
        for(int x=0;x<22;x++){
            ll temp=infinity;
            for(int y=0;y<22;y++){
                if(x!=y){
                   temp=min(temp,dp[child][y]);
                }
            }
            dp[node][x]+=temp;
        }
