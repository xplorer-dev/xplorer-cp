...
// here I have given n^4 approach and it works as constraints are n<=80
void solve()
{
    ll n;
    cin>>n;
    ...
    vector<vector<vector<ll>>> dp(n+1,vvl(n+1,vl(n+1,0)));
    for(int i=n;i>=0;i--){
        for(int j=i+1;j<n;j++){
            dp[i][j][2]+=1;
            for(int l = 2;l<=n-i;l++){
                for(int k=j+1;k<n;k++){
                    if( (v[k]-v[j])  == (v[j] - v[i]) ){
                        dp[j][k][2] = 1;
                        dp[i][j][l+1] = (dp[i][j][l+1] + dp[j][k][l])%M;
                    }
                }
            }
        }
